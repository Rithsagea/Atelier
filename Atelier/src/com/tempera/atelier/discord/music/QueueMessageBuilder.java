...
public class QueueMessageBuilder extends MessageBuilder {
	public QueueMessageBuilder(AtelierAudioHandler audioHandler,
		MenuManager menuManager, int page) {

		EmbedBuilder eb = new EmbedBuilder();
		eb.setColor(Color.CYAN);
		StringBuilder msg = new StringBuilder();
		int queueSize = audioHandler.getQueue()
			.size();
		...
		if (queueSize > 0) {
			...
			eb.setFooter(String.format("Displaying songs %d to %d out of %d",
				(page - 1) * 10 + 1, Math.min(page * 10, queueSize),
				queueSize));
			setActionRows(ActionRow.of(Button.primary("prev", "Previous"),
				Button.primary("next", "Next")));
		} else {
			eb.setTitle("No songs in queue!");
			setActionRows(ActionRow.of(Button.success("ha", "bad time")));
		}
