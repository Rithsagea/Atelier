...
public class MusicQueueMessageBuilder extends MessageBuilder {
	public MusicQueueMessageBuilder(AtelierAudioHandler audioHandler, int page) {
		...
		if (queueSize > 0) {
			...
			eb.setFooter(String.format("Displaying songs %d to %d out of %d",
				(page - 1) * 10 + 1, Math.min(page * 10, queueSize), queueSize));
