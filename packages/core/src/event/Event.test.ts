...
test("ignore removed event handlers and listeners", () => {
  ...
  emitter.addListener(listener);
  const handlerC = { method: fnC, priority: 0 };
  const handlerD = { method: fnD, priority: 0 };
  emitter.addHandler(TestEvent, handlerC);
  emitter.addHandler(TestEvent, handlerD);

  emitter.removeListener(listener);
  expect(emitter.removeHandler(TestEvent, handlerC)).toBeTrue();
  expect(emitter.removeHandler(TestEvent, handlerC)).toBeFalse();
...
test("call event handlers in priority order", () => {
  const TestEvent = createEvent();
  const arr: string[] = [];
...
test("clearHandlers removes all registered handlers", () => {
  const TestEvent = createEvent();
